...
void	print_more_infos2(t_files *data, t_recu **recu, char ***tab)
{
	t_recu *lst = *recu;
		...
		while (tab[i][j])
		{
			...
			// if (ft_strcmp(tab[i][j], ".") != 0 && ft_strcmp(tab[i][j], "..") != 0)
			// {
			// 	char *try = ft_strjoin(ft_strdup("./"), tab[i][j]);
			// 	if (ft_strcmp(lst->pwd, "./") == 0 || ft_strcmp(lst->pwd, "../") == 0)
			// 		ft_putstr(tab[i][j] + ft_strlen(lst->pwd));
			// 	else if (is_a_file(lst->pwd))
			// 		ft_putstr(tab[i][j]);
			// 	else
			// 		ft_putstr(tab[i][j] + ft_strlen(lst->pwd) + 1);
			// 	free(try);
			// }
			// else
			ft_putstr(tab[i][j]);
